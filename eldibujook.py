...
plt.subplots_adjust(left=0.48, bottom=0.45)  
...
# Textos formateados con saltos de línea estratégicos (\n) para que no invadan los deslizadores
labels_text = [
...
for y, k, label, r, fmt in zip(y_positions, keys, labels_text, ranges, formats):
    ...
    # Manejo del foco persistente al pulsar Enter / Confirmar valor
    def al_confirmar(text, k_key=k):
        text_boxes[k_key].ax.set_facecolor('white')  
        text_boxes[k_key].text_disp.set_color('black')  # Devuelve la tipografía a negro
...
def manejar_clics_foco(event):
    if event.inaxes is None:
        # Si se hace click fuera de cualquier control, limpiamos el foco de todos y guardamos cambios
        for k, tb in text_boxes.items():
            if tb.ax.get_facecolor() != (1.0, 1.0, 1.0, 1.0): # Si estaba seleccionado
    ...
    for k, tb in text_boxes.items():
        if event.inaxes == tb.ax:
            tb.ax.set_facecolor('#D0E2FF')      # Fondo azul claro de selección activa
            tb.text_disp.set_color('#0056B3')   # Texto azul oscuro que resalta qué valor se está editando
        else:
            if tb.ax.get_facecolor() != (1.0, 1.0, 1.0, 1.0): # Si estaba activo y clickamos otro sitio
                tb.ax.set_facecolor('white')
                tb.text_disp.set_color('black')
                actualizar(tb.text, 'text', k)
...
# Solución definitiva al error conectando el gestor de clics directo al lienzo global de la app
...
ax_export = plt.axes([0.55, 0.02, 0.2, 0.035])
